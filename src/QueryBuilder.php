...
use Modules\Table\Exceptions\UnresolveabeRelationException;
        ...
        $data = $columns
            ->mapWithKeys(function (Column $column) use ($model): array {
                ...
                if ($column instanceof ActionColumn) {
                    $actionUrls = collect($this->table->buildActions())
                        ->map(function (Action $action) use ($model): string|array|null {
                            ...
                            // Always return an array with hidden and disabled states
                            return array_merge(
